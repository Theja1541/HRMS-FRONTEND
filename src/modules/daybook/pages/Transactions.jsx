...
export default function Transactions() {
  ...
  const canCreate = hasPermission("daybook", "create", "transactions");
  const canEdit = hasPermission("daybook", "edit", "transactions");
  const canExport = hasPermission("daybook", "export", "transactions");
    ...
    <div className="employees-page">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Manage company transactions and cashbook ledger</p>
        </div>
        <div className="header-actions">
          {canExport && <button className="settings-btn" onClick={exportToExcel}>📊 Export Excel</button>}
          {canExport && <button className="deactivated-btn" onClick={exportToPDF}>📄 Export PDF</button>}
          {canCreate && (
            <button className="add-employee-btn" onClick={() => navigate("/daybook/transactions/add")}>
              + Add Transaction
            </button>
          )}
        </div>
      ...
      <div className="filter-bar">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <label style={{ fontSize: "13px", color: "#475569", fontWeight: "600" }}>From Date:</label>
          <input
            type="date"
            className="search-input"
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <label style={{ fontSize: "13px", color: "#475569", fontWeight: "600" }}>To Date:</label>
          <input
            type="date"
            className="search-input"
          ...
          <table className="employees-table">
            ...
            <tbody>
              {transactions.map((txn) => {
                ...
                const amount = isDebit ? txn.debit_amount : txn.credit_amount;
                const vendor = isDebit ? txn.to_vendor_name : txn.from_vendor_name;
                const gstRate = txn.gst_applicable && Number(amount) > 0 
                  ? `${((Number(txn.gst_amount) / Number(amount)) * 100).toFixed(1).replace(/\.0$/, '')}%` 
                  : "-";
                const totalAmount = Number(amount) + (txn.gst_applicable ? Number(txn.gst_amount) : 0);

                return (
                  <tr key={txn.id}>
                    <td><strong>{new Date(txn.date).toLocaleDateString('en-IN')}</strong></td>
                    <td style={{ fontWeight: "600", color: "#334155" }}>{txn.transaction_number || `REC-${txn.id}`}</td>
                    <td>{vendor || '-'}</td>
                    <td>
                      ...
                      <div className="action-buttons">
                        ...
                        {canEdit && (
                          <button
                            ...
                            onClick={() => navigate(`/daybook/transactions/edit/${txn.id}`)}
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/daybook/transactions/invoice/${txn.id}`)}
                          title="View Invoice"
                        >
                          📄 Invoice
                        </button>
                        <button
                          type="button"
                          className="action-btn"
                          style={{ background: "#faf5ff", color: "#6b21a8", border: "none" }}
                          onClick={() => navigate(`/daybook/transactions/receipt/${txn.id}`)}
                          title="View Receipt"
                        >
                          🧾 Receipt
                        </button>
        ...
        <div className="modal-overlay" onClick={() => setShowDetailsModal(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '600px', width: '90%' }}>
            ...
            <div className="modal-actions">
              <button className="btn primary" onClick={() => downloadReceipt(selectedTransaction)} style={{ marginRight: '10px' }}>📄 Download Receipt</button>
              <button className="btn" onClick={() => setShowDetailsModal(false)} style={{ background: '#64748b', color: 'white' }}>Close</button>
            </div>
          </div>
        </div>
      )}
